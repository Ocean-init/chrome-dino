  ...
  while( window.isOpen() ){
    ...
    while( const std::optional event = window.pollEvent()){
      ...
      if(const auto* keyPressed = event->getIf<sf::Event::KeyPressed>()){
        if(keyPressed->scancode == sf::Keyboard::Scancode::Up || keyPressed->scancode == sf::Keyboard::Scancode::Space && gravity == ground.getPosition().y - (dino.getTextureRect().size.y - gap)){
          velocity = jump;

        }

        if(keyPressed->scancode == sf::Keyboard::Scancode::Down && !is_crouching && gravity == ground.getPosition().y - (dino.getTextureRect().size.y - gap)){
          crouch = true;
          is_crouching = true;
    ...
    if(!gameover){
      ...
      if(count % (50 * rand_distance) == 0){
        ...
        if(sprite.getTextureRect().size.y == 68){
          ...
          sprite.setPosition(
		  {window.getSize().x,
		  520 - sprite.getTextureRect().size.y + gap}
              );
        }
      ...
      frame_ground -= 8.f;
      if(frame_ground < -ground.getTextureRect().size.x){
        frame_ground = 0.f;
      }
      ground.setPosition({frame_ground - speed, ground.getPosition().y});
      ground_back.setPosition({frame_ground - speed + (ground.getTextureRect().size.x - 40), ground.getPosition().y});
      ...
      for (size_t i {}; i < clouds.size(); ++i){
        ...
        if(clouds[i].getPosition().x < -clouds[i].getTextureRect().size.x){
